<template>
    ...
    export default {
      ...
      async mounted() {
        ...
        console.log('GetlessonIndexFromQuestions',GetlessonIndexFromQuestions);
      ...
      methods: {
        ...
        async selectRandomItems(data, count) {
          ...
          this.selectedItems = shuffled.slice(0, count).map(item => ({
            ...
            has_AddImg: item.has_AddImg,
            questions: item.questions,
            option: item.option,
          }));
          const lessonIndex = await this.playerData.lesson.findIndex(c => c.guid == this.lessonGuid);
          if(this.isRestartNewGame && this.IsExistPlayer && lessonIndex != -1){
            console.log('this.isRestartNewGame && this.IsExistPlayer && lessonIndex != -1',this.isRestartNewGame && this.IsExistPlayer && lessonIndex != -1);
        ...
        async SetPlayerData() {
          ...
          console.log("###SetPlayerData this.allPlayerData", this.allPlayerData);
        },
